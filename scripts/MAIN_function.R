...
MAIN_fun<-function(x = dir(x, all.files = T), ..deparseOpts){
  ...
  while (i<=10) { #заменить на while
    ...
    CS_subset(alignment = ali, cs_output = cs_out, PA_output = PA_out, aa_stat = aa_stplot,aa_stat_wNA=aa_stplot_na, cs_stat_comp = cs_comp_plot, summary = sum_st, summary_wNA = sumNA)
    i = i+1  
  }
}
